...
const connectUrl = computed(() => {
  ...
  return `https://vercel.com/integrations/tickup/new?redirect_url=${encodeURIComponent(redirectUri)}`;
});
</script>
...
<template>
  <v-row class="fill-height">
    <v-col cols="12">
      ...
      <v-card variant="flat" class="pa-4 mb-4">
        <v-card-text>
          ...
          <div v-else-if="connected" class="d-flex align-center justify-space-between">
            ...
            <v-btn variant="tonal" color="error" size="small" :loading="disconnecting" @click="disconnect">
              Disconnect
            </v-btn>
          </div>
          <div v-else>
            ...
            <v-btn v-if="connectUrl" color="primary" prepend-icon="mdi-triangle" :href="connectUrl">
              Connect Vercel
            </v-btn>
      ...
      <v-alert v-if="route.query.vercel === 'connected'" type="success" variant="tonal" class="mb-4">
        Vercel connected successfully.
      </v-alert>
      <v-alert v-if="route.query.vercel === 'error'" type="error" variant="tonal" class="mb-4">
