    ...
    <div class="mx-auto px-7 pt-7 pb-12" style="width: 100%">
      <div>
        ...
        <v-row no-gutters align="center" class="mb-4 flex-nowrap ga-3">
          ...
          <v-col cols="auto" class="d-flex align-center ga-3 flex-shrink-0">
            ...
            <div class="view-switcher pa-1 rounded-lg d-flex">
              <button v-for="v in views" :key="v.key" class="view-btn d-flex align-center ga-1 px-3"
                :class="{ 'view-btn--active': currentView === v.key }" @click="currentView = v.key">
                <v-icon size="15">{{ v.icon }}</v-icon>
                {{ v.label }}
              </button>
            </div>
          </v-col>
        ...
        <div class="add-task-bar d-flex align-center px-4 mb-5 ga-3">
          ...
          <input v-model="newTodoName" placeholder="Add a task…" class="add-task-input flex-grow-1"
