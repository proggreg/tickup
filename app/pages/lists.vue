...
<template>
    <div class="d-flex flex-column h-100 pb-20">
        <div class="d-flex align-center ga-3 px-5 pt-8 pb-4 flex-shrink-0">
            ...
            <v-chip v-if="store.lists.length" size="small" variant="tonal" label>
                {{ store.lists.length }}
            </v-chip>
        </div>
        ...
        >
            <v-icon icon="mdi-playlist-plus" size="56" class="text-disabled mb-3" />
            <p class="text-body-1 font-weight-medium text-medium-emphasis mb-1">No lists yet</p>
            <p class="text-body-2 text-disabled">Tap the button below to create your first list</p>
        </div>
