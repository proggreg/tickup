...
const rowRefs = ref<Array<{ startEdit: () => void }>>([]);
...
function addStatus() {
    ...
    if (prev?.name === '') return;
    const color = palette.value[Math.floor(Math.random() * palette.value.length)];
    store.userStatuses.splice(insertAt, 0, { name: '', color });
    nextTick(() => rowRefs.value[insertAt]?.startEdit());
}
</script>
<template>
