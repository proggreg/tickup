...
async function updateTodo(e: { added?: { element: Todo } }, status: Status) {
    if (e.added) {
        if (e.added.element.status !== status.name) {
            const index = listStore.currentList.todos.findIndex((todo: Todo) => todo.id === e.added?.element.id);
                    ...
                    >
                        <template #item="{ element }">
                            <v-card
                                ...
                                :max-width="$device.isMobile ? '100%' : cardWidth"
                            ...
                            <v-card class="px-4">
                                <v-card-item class="px-0">
                                    <v-text-field
                                        ...
                                        variant="plain"
                                        @blur="newTodo.status = ''"
                                        @keyup.enter.stop="addTodo(status.name)"
                                    />
                                </v-card-item>
                            </v-card>
                        </template>
