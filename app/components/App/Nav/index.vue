...
<template>
  ...
  <v-navigation-drawer v-if="loggedIn" :rail="smAndDown" :expand-on-hover="smAndDown" permanent class="font-weight-bold"
    data-testid="nav-bar">
    <v-list nav class="flex-shrink-0">
      <v-list-item prepend-icon="mdi-home" title="Home" to="/" />
      <v-list-item>
        <v-list-item prepend-icon="mdi-cog" title="Settings" class="pa-0" to="/settings">

        </v-list-item>
      </v-list-item>
      <v-list-item prepend-icon="mdi-plus" title="New List" data-testid="new-list-button" @click="
        dialog.page = 'list';
      dialog.open = true;
      " />
