...
<template>
  <div class="recent-card">
    ...
    <div class="recent-header">
      <div class="recent-header-left">
        <div class="icon-tile-sm accent-tile">
          <v-icon size="17" class="accent-deep-icon">
            mdi-history
          </v-icon>
        </div>
      ...
      <div class="search-box" :class="{ 'search-box--focused': searchFocused }">
        ...
        <input v-model="searchQuery" class="search-input" placeholder="Search name, list, branch or date…"
      ...
      <div ref="filterRef" class="list-filter">
        <button class="lf-btn" :class="{
          'lf-btn--active': filterActive,
          'lf-btn--open': filterOpen,
        }" @click="filterOpen = !filterOpen">
          <v-icon size="16" class="lf-icon">
            mdi-filter-variant
          </v-icon>
        ...
        <div v-if="filterOpen" class="lf-menu">
          <div class="lf-option" :class="{ 'lf-option--selected': listFilter === 'all' }" @click="selectFilter('all')">
            <span class="lf-option-label">All lists</span>
            <v-icon v-if="listFilter === 'all'" size="15" class="lf-check">
              mdi-check
            </v-icon>
          </div>
          <div v-for="list in sortedLists" :key="list.id" class="lf-option"
            :class="{ 'lf-option--selected': listFilter === list.id }" @click="selectFilter(list.id!)">
            <span class="lf-dot" />
    ...
    <div v-if="rows.length === 0 && (searchQuery || filterActive)" class="no-results">
      <v-icon size="40" class="no-results-icon">
      ...
      <div v-else class="no-results-body">
    ...
    <div v-else>
      ...
      <!-- Table rows -->
      <div v-for="(row, i) in rows" :key="row.todo.id" class="table-row"
        :class="{ 'table-row--last': i === rows.length - 1 }" @click="selectTodo(row.todo)">
        <!-- Todo cell -->
        <div class="todo-cell">
          <div class="row-check" :class="isTodoClosed(row.todo.status) ? 'row-check--done' : 'row-check--open'
          ...
          <div class="todo-cell-text">
            <div class="todo-name" :class="{ 'todo-name--done': isTodoClosed(row.todo.status) }">
              {{ row.todo.name }}
            </div>
            <div class="status-dot-row">
              <span class="status-dot" :class="`status-dot--${row.todo.status?.toLowerCase()}`" />
              <span class="status-label" :class="`status-label--${row.todo.status?.toLowerCase()}`">
                {{
