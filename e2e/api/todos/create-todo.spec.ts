...
test.describe('Todos', () => {
    ...
    test('should default status to open if no status is given', async ({ request }) => {
        const testId = uuidv4();
    ...
    test('should create a todo with a due date', async ({ request }) => {
        ...
        if (!todo.dueDate) throw Error('dueDate should be defined');

        expect(new Date(todo.dueDate).toDateString()).toEqual(dueDate.toDateString());
    });
    ...
    test('should return 400 when parentId does not exist', async ({ request }) => {
        const testId = uuidv4();
